use romap::{discard_values, union};
...
fn test() {
    for size in [0, 1, 10, 1000] {
        ...
        romap::test_utils::test_iter_eq(discard_values(&btree_map), &btree_set, false);
        ...
        romap::test_utils::test_iter_eq(discard_values(&hash_map), &hash_set, false);

        romap::test_utils::test_iter_eq(&hash_map, &btree_map, false);
        romap::test_utils::test_iter_eq(discard_values(&hash_set), &hash_set, true);

        romap::test_utils::test_map(union(&hash_map, &btree_map), &size);
        romap::test_utils::test_map(union(&hash_map, &BTreeMap::<usize, usize>::new()), &size);
    }
}
